...
class ColorfulCharcoalBlockTemplate extends StatelessWidget with InvoiceTemplateHelpers {
  @override final Invoice invoice;
  @override final BusinessInfo business;
  @override final String Function(double) fmtMoney;
  const ColorfulCharcoalBlockTemplate({super.key, required this.invoice, required this.business, required this.fmtMoney});

  @override
  Widget build(BuildContext context) {
    const accent = Color(0xFF263238); // Charcoal
    return Container(
      color: Colors.white,
  ...
  Widget buildItemsTableWithHeader(Color accent) {
    return Column(children: [
      ...invoice.items.map((item) => Container(
        ...
        child: Row(children: [
          Expanded(child: Text(item.description, style: const TextStyle(fontWeight: FontWeight.bold))),
          Text(fmtMoney(item.total), style: TextStyle(color: accent, fontWeight: FontWeight.w900)),
        ]),
      )),
    ]);
  }
