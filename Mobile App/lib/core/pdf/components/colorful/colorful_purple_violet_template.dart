...
class ColorfulPurpleVioletTemplate extends StatelessWidget with InvoiceTemplateHelpers {
  @override final Invoice invoice;
  @override final BusinessInfo business;
  @override final String Function(double) fmtMoney;
  const ColorfulPurpleVioletTemplate({super.key, required this.invoice, required this.business, required this.fmtMoney});

  @override
  Widget build(BuildContext context) {
    const accent = Color(0xFF673AB7); // Purple
    return Container(
      color: Colors.white,
  ...
  Widget _buildHeader(Color accent) {
    return Container(
      ...
      color: accent,
      child: const Center(child: Text('INVOICE', style: TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.bold))),
    );
  }
  ...
  Widget buildItemsTableWithHeader(Color accent) {
    return Column(children: [
      ...invoice.items.map((item) => Container(
        ...
        decoration: BoxDecoration(color: accent.withOpacity(0.05), borderRadius: BorderRadius.circular(8)),
        child: Row(children: [
  ...
  Widget buildTotalsWithAccent(Color accent) {
    return Container(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(fmtMoney(invoice.totalAmount), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 20)),
      ]),
    );
  }
}

// 8. Colorful Clean Mono