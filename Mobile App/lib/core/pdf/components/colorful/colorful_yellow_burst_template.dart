...
class ColorfulYellowBurstTemplate extends StatelessWidget with InvoiceTemplateHelpers {
  @override final Invoice invoice;
  @override final BusinessInfo business;
  @override final String Function(double) fmtMoney;
  const ColorfulYellowBurstTemplate({super.key, required this.invoice, required this.business, required this.fmtMoney});

  @override
  Widget build(BuildContext context) {
    const accent = Color(0xFFFFB300); // Yellow/Amber
    return Container(
      color: Colors.white,
    ...
    return Container(
      ...
      child: Stack(
        children: [
          Positioned(
            top: -40, left: -40,
            child: Container(width: 140, height: 140, decoration: BoxDecoration(color: accent.withOpacity(0.1), shape: BoxShape.circle)),
          ),
          Positioned(
            top: -20, right: -30,
            child: Container(width: 200, height: 200, decoration: BoxDecoration(color: accent.withOpacity(0.2), shape: BoxShape.circle)),
          ),
          Padding(
              ...
              children: [
                Column(
                  ...
                  children: [
                    ...
                    Text(business.name, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
                    Text(business.businessEmail ?? '', style: const TextStyle(fontSize: 10, color: AppColors.darkGrey)),
                  ],
                ),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: [
                    Text('INVOICE', style: TextStyle(color: accent, fontSize: 32, fontWeight: FontWeight.w900, letterSpacing: 2)),
                    Text('INV-${invoice.id}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                    Text(invoice.issueDate.toString().split(' ')[0], style: const TextStyle(fontSize: 10, color: AppColors.darkGrey)),
                  ],
                ),
              ],
            ),
        ...
        child: Column(
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              Text(fmtMoney(invoice.subtotal), style: const TextStyle(fontSize: 12)),
            ]),
            const Divider(),
            Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Text(fmtMoney(invoice.totalAmount), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
              ]),
            ),
          ],
        ),
